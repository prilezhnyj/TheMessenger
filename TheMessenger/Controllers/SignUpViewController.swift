...
class SignUpViewController: UIViewController {
    ...
    let forgotPasswordButton = UIButton(titleText: "Sign In", titleFont: SetupFont.sfProBold(sizeFont: 15), titleColor: .black, backgroundColor: .clear, cornerRadius: 0, isShadow: true, isBorder: false)
...
extension SignUpViewController {
    private func setupConstraints() {
        ...
        let emailView = MailView(label: emailLabel, textField: emailTextField)
        let passwordView = MailView(label: passwordLabel, textField: passwordTextField)
        let confirmPasswordView = MailView(label: confirmPasswordLabel, textField: confirmPasswordTextField)
